...
本文是 `bpmt-lite` v1.4.1 的 API 归档版文档，面向人类阅读。机器可读的 OpenAPI 快照见 [openapi.json](openapi.json)。
...
v1.4.1 开放动态表结构管理与数据库操作模块：

- 创建动态表结构。
- 查询动态表列表和详情。
- 调整动态表字段结构。
- 同步动态表 DDL。
- 查询动态表模板列表、模板详情和按模板建表。
- 受控 SQL 接口：`query/find/save/exec`。
